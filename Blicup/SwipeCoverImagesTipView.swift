...
@IBDesignable
class SwipeCoverImagesTipView: UIView {
    ...
    func startAnimation() {
        UIView.animateWithDuration(1, delay: 0.0, options: [UIViewAnimationOptions.CurveLinear], animations: {
            self.ivHand.transform = CGAffineTransformMakeTranslation(0, -40)
        }) { (_) in
    ...
    func stopAnimation() {
        UIView.animateWithDuration(0.5, delay: 0.0, options: [UIViewAnimationOptions.BeginFromCurrentState], animations: {
            ...
            NSUserDefaults.standardUserDefaults().setObject(["hasPerformedTip" : true], forKey: kSwipeCoverImagesTipKey)
        }
    }
