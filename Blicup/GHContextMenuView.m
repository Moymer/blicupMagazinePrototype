...
@property (nonatomic, strong) NSMutableArray* menuHighlightedItems;
@property (nonatomic, strong) NSMutableArray* menuHighlightedItemsTitle;
@property (nonatomic, strong) UILabel *itemHighlightedLabel;
@property (nonatomic, strong) UIView *snapView;
@property (nonatomic, strong) UIView *backgroundView;

@property (nonatomic) CGFloat radius;
@property (nonatomic) CGFloat radiusLabel;
@property (nonatomic) CGFloat paddingLabel;
...
- (id)init
{
    ...
    if (self) {
        ...
        _menuHighlightedItems = [NSMutableArray array];
        _menuHighlightedItemsTitle = [NSMutableArray array];
        ...
        _itemBGHighlightedColor = [[UIColor alloc] initWithRed:255.0/255.0 green:0 blue:102.0/255.0 alpha:1.0];
...
-(void)dismissWithSelectedIndexForMenuAtPoint:(CGPoint)point {
    ...
    if(self.delegate && [self.delegate respondsToSelector:@selector(getSnapViewInCollection:)]) {
        [self.snapView removeFromSuperview];
    }
    
    if(self.delegate && [self.delegate respondsToSelector:@selector(contextMenuWillHideforMenuAtPoint:)]) {
        [self.delegate contextMenuWillHideforMenuAtPoint:point];
    }
...
-(void)sendPositionToDelagate:(CGPoint)point {
    
    if(self.delegate && [self.delegate respondsToSelector:@selector(getSnapViewInCollection:)]) {
        _snapView = [self.delegate getSnapViewInCollection: point];
...
- (void) longPressDetected:(UIGestureRecognizer*) gestureRecognizer
{
    if (gestureRecognizer.state == UIGestureRecognizerStateBegan) {
        ...
        [self sendPositionToDelagate: pointInView];
        
        [[UIApplication sharedApplication].keyWindow addSubview:self.backgroundView];
        [[UIApplication sharedApplication].keyWindow addSubview:self];
        
        if(self.delegate && [self.delegate respondsToSelector: @selector(shouldAnimateTipPerformed)]) {
            [self.delegate shouldAnimateTipPerformed];
        }
...
- (void) hideMenu
{
    if (self.isShowing) {
        ...
        self.isPaning = NO;
        [self animateMenu:NO];
        [self.itemHighlightedLabel removeFromSuperview];
...
- (CALayer*) layerWithImage:(UIImage*) image
{
    ...
    UIBezierPath *shadowPath = [UIBezierPath bezierPathWithRect:UIEdgeInsetsInsetRect(layer.bounds, shadowInsets)];
...
- (void) reloadData
{
    ...
    [self.menuHighlightedItems removeAllObjects];
    [self.menuHighlightedItemsTitle removeAllObjects];
    ...
    if (self.dataSource != nil) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if ([self.dataSource respondsToSelector:@selector(highLightedImageForItemAtIndex:)]) {
                UIImage* highLightedImage = [self.dataSource highLightedImageForItemAtIndex:i];
                [self.menuHighlightedItems addObject:highLightedImage];
            }
            
            if ([self.dataSource respondsToSelector:@selector(highLightedTitleForItemAtIndex:)]) {
                [self.menuHighlightedItemsTitle addObject:[self.dataSource highLightedTitleForItemAtIndex:i]];
            }
        }
    }
}
...
- (void) layoutMenuItems
{
    ...
    self.arcAngle = (((itemRadius * self.menuItems.count) - 15)/ self.radius) * 1.5;
...
- (CGFloat) itemAngleAtIndex:(NSUInteger) index
{
    ...
    CGFloat xPosition = self.longPressLocation.x < self.center.x ? self.bounds.size.width : 0;
    CGFloat yPosition = self.longPressLocation.y < self.center.y ? self.bounds.size.height : 0;
    CGPoint endingPoint = CGPointMake(xPosition, yPosition);
    
    float bearingRadians = [self angleBeweenStartinPoint:self.longPressLocation endingPoint:endingPoint];
...
- (void)createLabel {
    ...
    if (![[self subviews] containsObject:self.itemHighlightedLabel]) {
        self.itemHighlightedLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width/2 - self.paddingLabel, 150)];
        [self addSubview:self.itemHighlightedLabel];
    }
    
    CGPoint origin = [self originForLabel];
    CGRect frame = self.itemHighlightedLabel.frame;
    frame.origin = origin;
    self.itemHighlightedLabel.frame = frame;
    self.itemHighlightedLabel.text = @"";
    self.itemHighlightedLabel.numberOfLines = 0;
    self.itemHighlightedLabel.lineBreakMode = NSLineBreakByWordWrapping;
    CGFloat size =  [[UIScreen mainScreen] bounds].size.width > 320 ? 50 : 40;
    self.itemHighlightedLabel.font = [UIFont fontWithName:@"SFUIText-Bold" size:size];
    self.itemHighlightedLabel.textColor = [UIColor whiteColor];
...
- (CGPoint) originForLabel
{
    ...
    CGFloat xPosition = self.longPressLocation.x < self.center.x ? self.center.x : self.paddingLabel;
    CGFloat yPosition = self.longPressLocation.y < self.center.y ? self.longPressLocation.y + self.radiusLabel : self.longPressLocation.y - self.radiusLabel - 100;
    self.itemHighlightedLabel.textAlignment =  self.longPressLocation.x < self.center.x ? NSTextAlignmentRight : NSTextAlignmentLeft;
    CGPoint origin = CGPointMake(xPosition, yPosition);
    ...
    if (self.isShowing && self.isPaning) {
        ...
        if (closeToIndex >= 0 && closeToIndex < self.menuItems.count) {
            ...
            if (fabs(distanceFromItem) < toleranceDistance ) {
                ...
                if ([self.dataSource respondsToSelector:@selector(highLightedTitleForItemAtIndex:)]) {
                    self.itemHighlightedLabel.text = [self.menuHighlightedItemsTitle objectAtIndex:closeToIndex];
                }
                
                if ([self.dataSource respondsToSelector:@selector(highLightedImageForItemAtIndex:)]) {
                    CALayer *imageLayer = layer.sublayers.firstObject;
                    UIImage *highlightedImage = [self.menuHighlightedItems objectAtIndex:closeToIndex];
                    imageLayer.contents = (id) highlightedImage.CGImage;
                }
...
- (void) resetPreviousSelection
{
    if (self.prevIndex >= 0) {
        ...
        layer.transform = CATransform3DIdentity;
        CALayer *imageLayer = layer.sublayers.firstObject;
        UIImage* image = [self.dataSource imageForItemAtIndex:self.prevIndex];
        imageLayer.contents = (id) image.CGImage;
        self.itemHighlightedLabel.text = @"";
        self.prevIndex = -1;
    }
...
- (void)animationDidStart:(CAAnimation *)anim
{
    if([anim valueForKey:GHShowAnimationID]) {
        ...
        if (self.itemLocations.count > 0) {
            ...
            [UIView animateWithDuration:GHAnimationDuration animations:^{
                _backgroundView.alpha = toAlpha;
            }];
        }
...
- (void)drawCircle:(CGPoint)locationOfTouch
{
    ...
    CGContextSetLineWidth(ctx, GHBorderWidth);
    CGContextSetRGBStrokeColor(ctx,1,0,102.0/255.0,1.0);
